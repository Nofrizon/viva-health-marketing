...
-- Allow anon access
DO $$ BEGIN
  CREATE POLICY "Enable all for anon" ON public.stores FOR ALL TO anon USING (true) WITH CHECK (true);
EXCEPTION WHEN others THEN NULL;
END $$;

DO $$ BEGIN
  CREATE POLICY "Enable all for anon" ON public.store_keywords FOR ALL TO anon USING (true) WITH CHECK (true);
EXCEPTION WHEN others THEN NULL;
END $$;
